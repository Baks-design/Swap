#ifndef VOLUMETRIC_FOG_HLSL
#define VOLUMETRIC_FOG_HLSL
...
float HenyeyGreenstein(float cosTheta, float g2, float gPow2)
{
    ...
    return (denominator != 0.0) ? (1.0 - gPow2) / (4.0 * PI * pow(abs(denominator), 1.5)): 0.0;
}
    ...
    out float4 output)
{
    ...
    for (uint i = 0; i < MAX_RAYMARCH_STEPS; i++)
    {
        if (i >= raymarchSteps)
        {
            break;
