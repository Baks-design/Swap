...
{
    public enum RigidbodyInteractionType { None, Kinematic, SimulatedDynamic }
    public enum StepHandlingMethod { None, Standard, Extra }
    public enum MovementSweepState { Initial, AfterFirstHit, FoundBlockingCrease, FoundBlockingCorner, }
    ...
    public class KinematicCharacterMotor : MonoBehaviour
    {
        ...
        PhysicsMaterial CapsulePhysicsMaterial;
        ...
        bool _movePositionDirty = false;
        Vector3 _movePositionTarget = Vector3.zero;
        bool _moveRotationDirty = false;
        Quaternion _moveRotationTarget = Quaternion.identity;
        int _rigidbodyProjectionHitCount = 0;
        bool _isMovingFromAttachedRigidbody = false;
        bool _mustUnground = false;
        float _mustUngroundTimeCounter = 0f;
        Vector3 _cachedWorldUp = Vector3.up;
        Vector3 _cachedWorldForward = Vector3.forward;
        Vector3 _cachedWorldRight = Vector3.right;
        Vector3 _cachedZeroVector = Vector3.zero;
        Quaternion _transientRotation;
        Transform _transform;
        Vector3 _transientPosition;
        Vector3 _characterUp;
        Vector3 _characterForward;
        Vector3 _characterRight;
        Vector3 _initialSimulationPosition;
        Quaternion _initialSimulationRotation;
        Rigidbody _attachedRigidbody;
        Vector3 _characterTransformToCapsuleCenter;
        Vector3 _characterTransformToCapsuleBottom;
        Vector3 _characterTransformToCapsuleTop;
        Vector3 _characterTransformToCapsuleBottomHemi;
        Vector3 _characterTransformToCapsuleTopHemi;
        Vector3 _attachedRigidbodyVelocity;
        int _overlapsCount;
        readonly OverlapResult[] _overlaps = new OverlapResult[MaxRigidbodyOverlapsCount];
        ...
        /// <summary>
        /// Moves the character position, taking all movement collision solving int account. 
            ...
            if (InteractiveRigidbodyHandling)
            {
                ...
                if (_attachedRigidbody)
                    GetVelocityFromRigidbodyMovement(
                        _attachedRigidbody, _transientPosition, deltaTime, out tmpVelocityFromCurrentAttachedRigidbody,
                ...
                if (_attachedRigidbody)
                {
                    ...
                    var newForward =
                        Vector3.ProjectOnPlane(
                            Quaternion.Euler(
                                deltaTime * Mathf.Rad2Deg * tmpAngularVelocityFromCurrentAttachedRigidbody) * _characterForward, _characterUp).normalized;
                ...
                if (GroundingStatus.GroundCollider && GroundingStatus.GroundCollider.attachedRigidbody &&
                    GroundingStatus.GroundCollider.attachedRigidbody == _attachedRigidbody && _attachedRigidbody != null &&
        ...
        public void UpdatePhase2(float deltaTime)
        {
            ...
            if (_solveMovementCollisions && InteractiveRigidbodyHandling)
            {
                if (InteractiveRigidbodyHandling)
                {
                    // Solve potential attached rigidbody overlap
                    if (_attachedRigidbody)
                    {
                        var upwardsOffset = Capsule.radius;

                        if (CharacterGroundSweep(
                            _transientPosition + (_characterUp * upwardsOffset), _transientRotation, -_characterUp,
                            upwardsOffset, out var closestHit))
                        {
                ...
                if (InteractiveRigidbodyHandling)
                {
                    // Resolve overlaps that could've been caused by rotation or physics movers simulation pushing the character
                    var iterationsMade = 0;
                    var overlapSolved = false;
                    while (iterationsMade < MaxDecollisionIterations && !overlapSolved)
                    {
                        ...
                        if (nbOverlaps > 0)
                        {
                            for (var i = 0; i < nbOverlaps; i++)
                            {
                                ...
                                // Resolve overlaps that could've been caused by rotation or physics movers simulation pushing the character
                                if (Physics.ComputePenetration(
                                    Capsule, _transientPosition, _transientRotation, _internalProbedColliders[i], overlappedTransform.position,
                                    overlappedTransform.rotation, out var resolutionDirection, out var resolutionDistance))
                                {
                                    // Resolve along obstruction direction
                                    var mockReport = new HitStabilityReport() { IsStable = IsStableOnNormal(resolutionDirection) };
                                    resolutionDirection = GetObstructionNormal(resolutionDirection, mockReport.IsStable);
                                    ...
                                    if (InteractiveRigidbodyHandling)
                                    {
                                        ...
                                        {
                                            var tmpReport = new HitStabilityReport() { IsStable = IsStableOnNormal(resolutionDirection) };
                                            ...
                                            if (probedRigidbody != _attachedRigidbody)
                                            {
                                                ...
                                                StoreRigidbodyHit(
                                                    probedRigidbody, Velocity, estimatedCollisionPoint, resolutionDirection, tmpReport);
                                            }
                                        }
                                    }
            ...
            // Calculate Character movement from base velocity   
        ...
        /// </summary>
        bool IsStableOnNormal(Vector3 normal) => Vector3.Angle(_characterUp, normal) <= MaxStableSlopeAngle;
        ...
        public void ProbeGround(
            ref Vector3 probingPosition, Quaternion atRotation, float probingDistance, ref CharacterGroundingReport groundingReport)
        {
            ...
            while (groundProbeDistanceRemaining > 0 && (groundSweepsMade <= MaxGroundingSweepIterations) && !groundSweepingIsOver)
            {
                ...
                if (CharacterGroundSweep(
                    groundSweepPosition, atRotation, groundSweepDirection, groundProbeDistanceRemaining, out var groundSweepHit))
                {
                    ...
                    EvaluateHitStability(
                        groundSweepHit.collider, groundSweepHit.normal, groundSweepHit.point, targetPosition,
                        _transientRotation, BaseVelocity, ref groundHitStabilityReport);
                    ...
                    else
                    {
                        ...
                        var sweepMovement = (groundSweepDirection * groundSweepHit.distance) +
                        ...
                        groundProbeDistanceRemaining = Mathf.Min(
                            GroundProbeReboundDistance, Mathf.Max(groundProbeDistanceRemaining - sweepMovement.magnitude, 0f));
        ...
        public Vector3 SetTransientPosition(Vector3 set) => _transientPosition = set;
        ...
        bool InternalCharacterMove(ref Vector3 transientVelocity, float deltaTime)
        {
            ...
            for (var i = 0; i < _overlapsCount; i++)
            {
                ...
                if (Vector3.Dot(remainingMovementDirection, overlapNormal) < 0f)
                {
                    ...
                    InternalHandleVelocityProjection(
                        stableOnHit, overlapNormal, obstructionNormal, originalVelocityDirection, ref sweepState,
                        previousHitIsStable, previousVelocity, previousObstructionNormal, ref transientVelocity,
                        ref remainingMovementMagnitude, ref remainingMovementDirection);
            ...
            while (remainingMovementMagnitude > 0f && (sweepsMade <= MaxMovementIterations) && hitSomethingThisSweepIteration)
            {
                ...
                {
                    var numOverlaps = CharacterCollisionsOverlap(tmpMovedPosition, _transientRotation, _internalProbedColliders, 0f, false);
                    if (numOverlaps > 0)
                    {
                        ...
                        for (var i = 0; i < numOverlaps; i++)
                        {
                            ...
                            if (Physics.ComputePenetration(
                                Capsule, tmpMovedPosition, _transientRotation, tmpCollider, tmpCollider.transform.position,
                                tmpCollider.transform.rotation, out var resolutionDirection, out var resolutionDistance))
                            {
                ...
                if (!foundClosestHit && CharacterCollisionsSweep(
                    tmpMovedPosition, _transientRotation, remainingMovementDirection, remainingMovementMagnitude + CollisionOffset,
                    out var closestSweepHit, _internalCharacterHits) > 0)
                {
                ...
                if (foundClosestHit)
                {
                    ...
                    EvaluateHitStability(
                        closestSweepHitCollider, closestSweepHitNormal, closestSweepHitPoint, tmpMovedPosition,
                        _transientRotation, transientVelocity, ref moveHitStabilityReport);
                    ...
                    if (_solveGrounding && StepHandling != StepHandlingMethod.None && moveHitStabilityReport.ValidStepDetected)
                    {
                        ...
                        if (obstructionCorrelation <= CorrelationForVerticalObstruction)
                        {
                            ...
                            var stepCastStartPoint = tmpMovedPosition + (stepForwardDirection * SteppingForwardDistance) + (_characterUp * MaxStepHeight);

                            // Cast downward from the top of the stepping height
                            var nbStepHits = CharacterCollisionsSweep(
                                stepCastStartPoint, _transientRotation, -_characterUp, MaxStepHeight,
                                out _, _internalCharacterHits, 0f, true);
                    ...
                    if (!foundValidStepHit)
                    {
                        ...
                        {
                            StoreRigidbodyHit(
                                closestSweepHitCollider.attachedRigidbody, transientVelocity,
                        ...
                        InternalHandleVelocityProjection(
                            stableOnHit, closestSweepHitNormal, obstructionNormal, originalVelocityDirection,
                            ref sweepState, previousHitIsStable, previousVelocity, previousObstructionNormal, ref transientVelocity,
                            ref remainingMovementMagnitude, ref remainingMovementDirection);
                    }
                }
                // If we hit nothing...
        ...
        /// </summary>
        void StoreRigidbodyHit(
            Rigidbody hitRigidbody, Vector3 hitVelocity, Vector3 hitPoint, Vector3 obstructionNormal, HitStabilityReport hitStabilityReport)
        {
            ...
            ref Vector3 transientVelocity, ref float remainingMovementMagnitude, ref Vector3 remainingMovementDirection)
        {
            ...
            else
            {
                ...
                {
                    EvaluateCrease(
                        transientVelocity, previousVelocity, obstructionNormal, previousObstructionNormal, stableOnHit,
                        previousHitIsStable, GroundingStatus.IsStableOnGround && !MustUnground(), out var foundCrease, out var creaseDirection);
        ...
        void EvaluateCrease(
            Vector3 currentCharacterVelocity, Vector3 previousCharacterVelocity, Vector3 currentHitNormal, Vector3 previousHitNormal,
            ...
            for (var i = 0; i < _rigidbodyProjectionHitCount; i++)
            {
                ...
                if (bodyHit.Rigidbody && !_rigidbodiesPushedThisMove.Contains(bodyHit.Rigidbody))
                {
                    if (_internalRigidbodyProjectionHits[i].Rigidbody != _attachedRigidbody)
                    {
                        ...
                        {
                            _ = hitCharacterMotor.SimulatedCharacterMass;
                        ...
                        ComputeCollisionResolutionForHitBody(
                            bodyHit.EffectiveHitNormal, characterVelocity, hitBodyVelocity, characterToBodyMassRatio,
                            out var velocityChangeOnCharacter, out var velocityChangeOnBody);
        ...
        public void ComputeCollisionResolutionForHitBody(
            Vector3 hitNormal, Vector3 characterVelocity, Vector3 bodyVelocity, float characterToBodyMassRatio,
        ...
        public void EvaluateHitStability(
            Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint, Vector3 atCharacterPosition, Quaternion atCharacterRotation,
            Vector3 withCharacterVelocity, ref HitStabilityReport stabilityReport)
        {
            ...
            if (LedgeAndDenivelationHandling)
            {
                ...
                if (CharacterCollisionsRaycast(
                    hitPoint + (atCharacterUp * SecondaryProbesVertical) + (innerHitDirection * SecondaryProbesHorizontal), -atCharacterUp,
                    ledgeCheckHeight + SecondaryProbesVertical, out var innerLedgeHit, _internalCharacterHits) > 0)
                {
                ...
                if (CharacterCollisionsRaycast(
                    ...
                    -atCharacterUp, ledgeCheckHeight + SecondaryProbesVertical, out var outerLedgeHit, _internalCharacterHits) > 0)
                {
        ...
        void DetectSteps(
            Vector3 characterPosition, Quaternion characterRotation, Vector3 hitPoint, Vector3 innerHitDirection, ref HitStabilityReport stabilityReport)
        {
            ...
            var stepCheckStartPos = hitPoint - verticalCharToHit + (characterUp * MaxStepHeight) + (3f * CollisionOffset * horizontalCharToHitDirection);

            // Do outer step check with capsule cast on hit point
            var nbStepHits = CharacterCollisionsSweep(
                stepCheckStartPos, characterRotation, -characterUp, MaxStepHeight + CollisionOffset, out _, _internalCharacterHits, 0f, true);
        ...
        bool CheckStepValidity(
            int nbStepHits, Vector3 characterPosition, Quaternion characterRotation, Vector3 innerHitDirection,
            Vector3 stepCheckStartPos, out Collider hitCollider)
        {
            hitCollider = null;
        ...
        /// </summary>
        public void GetVelocityFromRigidbodyMovement(
            Rigidbody interactiveRigidbody, Vector3 atPoint, float deltaTime,
        ...
        => GetVelocityFromMovement(toPosition - fromPosition, deltaTime);
        ...
        public int CharacterCollisionsOverlap(
            Vector3 position, Quaternion rotation, Collider[] overlappedColliders, float inflate = 0f,
        ...
        /// </summary>
        /// <returns> Returns number of overlaps </returns>
        public int CharacterOverlap(
            Vector3 position, Quaternion rotation, Collider[] overlappedColliders, LayerMask layers,
            QueryTriggerInteraction triggerInteraction, float inflate = 0f)
        {
            ...
            var nbUnfilteredHits = Physics.OverlapCapsuleNonAlloc(
                bottom, top, Capsule.radius + inflate, overlappedColliders, layers, triggerInteraction);
        ...
        public int CharacterCollisionsSweep(
            Vector3 position, Quaternion rotation, Vector3 direction, float distance, out RaycastHit closestHit,
            RaycastHit[] hits, float inflate = 0f, bool acceptOnlyStableGroundLayer = false)
        {
            var queryLayers = CollidableLayers;
            if (acceptOnlyStableGroundLayer)
                queryLayers = CollidableLayers & StableGroundLayers;
            ...
            var nbUnfilteredHits = Physics.CapsuleCastNonAlloc(
                bottom, top, Capsule.radius + inflate, direction, hits, distance + SweepProbingBackstepDistance,
        ...
        /// </summary>
        /// <returns> Returns the number of hits </returns>
        public int CharacterCollisionsRaycast(
            Vector3 position, Vector3 direction, float distance, out RaycastHit closestHit,
            RaycastHit[] hits, bool acceptOnlyStableGroundLayer = false)
        {
            ...
            var nbUnfilteredHits = Physics.RaycastNonAlloc(
                position, direction, hits, distance, queryLayers, QueryTriggerInteraction.Ignore);
